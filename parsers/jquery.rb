  ...
  def self.process_first(node, tree)
    ...
    if node.type == SyntaxTree::CONSTS["OBJECT_INIT"]
      if node.parent.type == SyntaxTree::CONSTS["LIST"]
        # extend
        if node.parent.parent.nodes[0].nodes[1] && node.parent.parent.nodes[0].nodes[1].name == "extend" &&
            (node.parent.parent.nodes[0].nodes[0].name == "$" || node.parent.parent.nodes[0].nodes[0].name == "jQuery" || node.parent.parent.nodes[0].nodes[0].name =~ /(?:$|jQuery)\.\w+/)
