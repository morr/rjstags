require 'enumerator'
require 'rbnarcissus/data'
require 'rbnarcissus/tokenizer'
require 'rbnarcissus/node'
...
module Narcissus
  ...
  def self.reduce(operators, operands, t)
    #pp "reduce "+operators.inject("") {|result,item| result += TOKENS[item.type]+" " }+operands.inject("") {|result,item| result += item.value+"*" }
    n = operators.pop
    op = n.type
    arity = OP_ARITY[op]
    if arity == -2
    ...
    #pp [arity, TOKENS[n.type]]

    # Always use push to add operands to n, to update start and end.
    raise SyntaxError.new("Internal parser error", n) unless arity
    ...
    operands.push(n)
    #pp "result "+operands.inject("") {|result,item| result += item.value+"*" }
    return n
  end
      ...
      gotoloopContinue = catch(:gotoloop) do
        ...
        while (tt = t.get) != CONSTS["END"]
          ...
          when CONSTS["ASSIGN"], CONSTS["HOOK"], CONSTS["COLON"]
            ...
            # dirty fix
            # i don't know how to fix bug correctly so i just skip all tokens until i meet ;
            #if TOKENS[tt] == "?"
              #operators = []
              #while (tt = t.get) != CONSTS["SEMICOLON"]
                ##pp [tt, TOKENS[tt]]
              #end
              #break
            #end
